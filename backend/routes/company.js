...
router.post('/', authenticate, async (req, res) => {
  const { name, industry, description, image_url } = req.body;
  try {
    ...
    const [company] = await db('companies')
      .insert({ user_id: req.user.id, name, industry, description, image_url })
      .returning('*');
...
router.put('/', authenticate, async (req, res) => {
  ...
  try {
    const updated = await db('companies')
      ...
      .update({ name, industry, description, image_url })
      .returning('*');
